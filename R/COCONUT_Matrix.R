...
COCONUT.matrix <- function (dat, batch, control, par.prior=TRUE,
  ...
  ComBatcontrol  <- .CombatCustom.matrix(dat=dat[,control==1], 
                                  params = "get", 
                                  batch = batch[control==1],
                                  par.prior=par.prior,
                                  itConv=itConv,
                                  parallel=parallel, 
                                  mc.cores=mc.cores)
  ...
  GSEs.disease.ComBat <- .CombatCustom.matrix(dat=dat[,control==0], 
                                       batch=batch[control==0],
                                       params = "have", 
                                       bayesParams = ComBatcontrol$bayesParams,
                                       ctrBatch = ComBatcontrol$ctrBatch)
  res=cbind(ComBatcontrol$bayesdata,GSEs.disease.ComBat)
...
.CombatCustom.matrix <- function(dat, 
                          params, 
                          batch,
                          bayesParams, 
                          par.prior=TRUE, 
  ...
  if(params == "get"){
    ComBatWithParams <- .ComBatGetParamsNoCov(dat, batch=batch,
                                              par.prior=par.prior,
                                              itConv=itConv,
                                              parallel=parallel,
                                              mc.cores=mc.cores)
    
    ComBatWithParams$ctrBatch <- unique(batch)
    
    return(ComBatWithParams)
    
  } else if (params == "have"){
    ## check order of platforms
    if(!(identical(unique(batch), ctrBatch))) {
      print(unique(batch))
      print(ctrBatch)
    ...
    commonGenesCombat <- .ComBatApplyParamsNoCov(dat,
                                                 batch = batch,
                                                 bayesParams = bayesParams)

    return(commonGenesCombat)
  }
}
